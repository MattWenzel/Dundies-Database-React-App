...
const ProductsOrderedUpdateForm = (props) => {
        ...
        <>
            <form className="flex flex-col justify-center items-center mt-10">
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div>
                        <div className="block">Product Order ID</div>
                        <select
                            className="border-4 rounded py-2 px-3" value={formData.product_order_id} onChange={(e) => {
                                setFormData({ ...formData, product_order_id: e.target.value });
                                props.selectHandler(e.target.value);
                            }}>
                            <option value="" disabled selected>-- Select Order ID --</option>
                            {props.productOrderData.map(option => <option value={option.product_order_id}>{option.product_order_id}</option>)}
                        </select>
                    </div>
                    <div>
...
export default ProductsOrderedUpdateForm;